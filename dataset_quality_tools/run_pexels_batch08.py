#!/usr/bin/env python3
"""Run the Pexels download pipeline (batch08) and notify via Feishu/Lark.
...
OUTPUT = Path("./pexels_nature_batch08")
LOG_FILE = Path("./pexels_nature_batch08.log")
...
def main() -> int:
    ...
    with LOG_FILE.open("w", encoding="utf-8") as log_fh:
        try:
            ...
            msg = (
                f"Pexels batch08 failed with exit code {exc.returncode}\n"
                f"Output: {OUTPUT.resolve()}\n"
            ...
            msg = (
                f"Pexels batch08 completed successfully\n"
                f"Output: {OUTPUT.resolve()}\n"
                f"End: {end_time}\n"
                f"Log: {LOG_FILE.resolve()}"
            )
